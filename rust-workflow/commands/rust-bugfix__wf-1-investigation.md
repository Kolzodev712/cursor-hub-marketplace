---
name: "Bug investigation (WF Step 1)"
description: "Investigate non-trivial bug: reproduction, evidence, scope, hypotheses; record in design log."
---
...
**Follow with:** Bugfix Step 2 — Proposed solution (`/rust-bugfix__wf-2-proposed-solution`), then Step 3 — Resolution (`/rust-bugfix__wf-3-resolution`).

## Steps

1. **Reproduce:** Confirm the bug from issue, user description, or stack trace. Document steps to reproduce and environment (e.g. branch, flags, inputs).
...
6. **Record in design log (mandatory):** Create or update the design log for this bugfix workstream. If no log exists, run `python .cursor/tools/new_design_log.py --slug bugfix-<short-name>` (or fallback: list `./.cursor/design-log/`, take max NNN + 1, create the file). Append (or fill) an **Investigation** section: reproduction steps, evidence, scope, hypotheses. Create logs only in `./.cursor/design-log/`. Do not ask — do it.
...
This workflow uses its own design logs; it does not assume a log from rust-design-review or rust-implementation.
