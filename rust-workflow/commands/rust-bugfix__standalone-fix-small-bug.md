---
name: "Fix small bug"
description: "Identify and fix a small, localized bug with minimal workflow."
---
...
2. **Root cause:** Narrow down the cause (e.g. off-by-one, missing check, wrong type). Keep scope minimal.
3. **Fix:** Make the minimal code change that fixes the bug. Do not refactor unrelated code.
4. **Verify:** Run `cargo test --all-features`, and optionally `cargo fmt --all`, `cargo clippy --fix --allow-dirty --all-targets --all-features -- -D warnings`. Fix any failures.
