---
name: "Bugfix resolution (WF Step 3)"
description: "Implement the fix and verify; minimal change aligned with proposal."
---
# Systematic resolution (Bugfix WF Step 3)

**Use after:** Bugfix Step 2 — Proposed solution (`/rust-bugfix__wf-2-proposed-solution`). Implement the fix and verify; keep the change minimal and aligned with the proposal.
...
3. **Verification:** Run `cargo fmt --all -- --check` (then `cargo fmt --all` if needed), `cargo clippy --fix --allow-dirty --all-targets --all-features -- -D warnings`, `cargo test --all-features`. Optionally `cargo audit` and `cargo check --all-targets --all-features` if the project uses them. Fix any failures.
