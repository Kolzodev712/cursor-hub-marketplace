# Implement module (js-ts-implementation WF Step 1)
...
4. **Per phase:**
   ...
   - Run verification: lint (e.g. `npm run lint` or `eslint .`), type check if used (e.g. `tsc --noEmit` or `npm run typecheck`), and tests (`npm test`). Fix any failures.
