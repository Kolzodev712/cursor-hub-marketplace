# Add tests only (js-ts-testing WF Step 1)
...
2. **Tests only:** Add or edit only test code and test fixtures (e.g. in `tests/`, `__tests__/`, or test modules). Do not change production code.
...
4. **Verification:** Run `npm test`. Ensure no existing tests break. Run lint (e.g. `npm run lint`) and type check if used on the test code if the project uses them.
