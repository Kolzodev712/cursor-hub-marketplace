...
2. **Plan steps:** Define 3–6 steps so that after each step the project still passes lint/typecheck/tests. Prefer additive steps (new interfaces, then migrate call sites, then remove old) over big-bang edits.
3. **Execute step by step:**
   - For each step: edit only the files needed for that step.
   - Run tests (`npm test`) and any lint/typecheck the project uses after the step.
   - If tests fail, fix before continuing.
   - Optionally show a one-line summary or short diff summary of what changed in that step.
4. **Final check:** Run full verification (lint, type check if used, npm test). Summarize the refactor outcome.
