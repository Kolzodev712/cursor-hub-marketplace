...
**Use after:** Bugfix Step 1 — Investigation (`/terraform-bugfix__wf-1-investigation`). Propose how to fix the bug and document trade-offs before implementing.

**Follow with:** Bugfix Step 3 — Resolution (`/terraform-bugfix__wf-3-resolution`).

## Steps

1. **Context:** Briefly reference the investigation (evidence, root cause, scope). If a design log exists for this bugfix, cite it.
2. **Proposed solution:** Describe the fix: what will change, where, and why it addresses the root cause. Be specific (files, resources, state impact).
3. **Alternatives:** Note 0–2 alternative approaches and why they were not chosen.
4. **Trade-offs:** Call out any trade-offs: state changes, destroy/recreate risk, backward compatibility.
5. **Verification plan:** How we will verify the fix (terraform validate, plan, no unintended destroy).
