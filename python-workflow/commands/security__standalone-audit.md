...
2. **Review:** Walk through code vulnerabilities, dependencies (run the ecosystem’s audit tool, e.g. `pip audit`, `npm audit`, if available), secrets and auth, crypto usage, misconfiguration, and CI/CD. Classify each finding by severity.
...
- If the project has a lockfile or manifest, run the ecosystem’s audit tool (e.g. `pip audit`, `npm audit`) and include results in the report.
